...
class Calculator extends React.Component {
    ...
    handleEvaluate = () => {
        if (!this.state.currentVal.includes("Limit")) {
            ...
            const answer = Math.round(1e12 * eval(expression)) / 1e12;
        ...
        if (!this.state.currentVal.includes("Limit")) {
            ...
            if (currentVal.length > 21) {
                ...
                if (evaluated) {
                    ...
                    this.setState({
                        currentVal: "0" === currentVal || isOperator.test(currentVal) ? value : currentVal + value,
                        formula: "0" === currentVal && "0" === value ? "" === formula ? value : formula : /([^.0-9]0|^0)$/.test(formula) ? formula.slice(0, -1) + value : formula + value
                    });
                }
            }
        }
    ...
    handleDecimal = () => {
        if (this.state.evaluated) {
            ...
            if (this.state.currentVal.length > 21) {
                ...
                if (endsWithOperator.test(this.state.formula) || ("0" === this.state.currentVal && "" === this.state.formula)) {
                    this.setState({
                        currentVal: "0.",
